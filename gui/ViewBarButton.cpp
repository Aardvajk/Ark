...
Cache::Cache(ViewBar *bar, ViewBarButton *button, const QString &text, const QPixmap &pix) : type(bar->type()), anim(new QPx::UnitAnimation(200, 800, button)), menu(nullptr), down(false), within(false), checkable(false), checked(false), text(text), pix(pix)
{
...
void ViewBarButton::setMenu(QMenu *menu)
{
